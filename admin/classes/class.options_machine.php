...
class Options_Machine {
		...
		foreach ($options as $value) {
			...
			//Start Heading
			 if ( $value['type'] != "heading" )
			 {
				...
				$fold='';
				if ( array_key_exists("fold", $value ) && isset( $smof_data[$value['fold']] ) ) {
			...
			switch ( $value['type'] ) {
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ', 'office').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
	...
	public static function optionsframework_uploader_function($id,$std,$mod){
		...
		$uploader .= '<div class="upload_button_div"><span class="button image_upload_button" id="'.$id.'">'.__('Upload','office').'</span>';
		
		if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
		$uploader .= '<span class="button image_reset_button '. $hide.'" id="reset_'. $id .'" title="' . $id . '">'. __('Remove','office') .'</span>';
		$uploader .='</div>' . "\n";
